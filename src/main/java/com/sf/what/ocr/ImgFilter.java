...
public class ImgFilter {
	...
	public ImgFilter(BufferedImage image) {
		this.image = image;
	...
	public BufferedImage changeGrey() {
		PixelGrabber pg = new PixelGrabber(this.image.getSource(), 0, 0,
				this.iw, this.ih, this.pixels, 0, this.iw);
		try {
			pg.grabPixels();
		...
		Image tempImg = Toolkit.getDefaultToolkit()
				.createImage(
						new MemoryImageSource(this.iw, this.ih, this.pixels, 0,
								this.iw));
		this.image = new BufferedImage(tempImg.getWidth(null),
				tempImg.getHeight(null), 4);
	...
	public BufferedImage getMedian() {
		...
		for (int i = 1; i < this.ih - 1; i++) {
			for (int j = 1; j < this.iw - 1; j++) {
				...
				this.pixels[(i * this.iw + j)] = (alpha << 24 | red << 16
						| green << 8 | blue);
			}
		}
		Image tempImg = Toolkit.getDefaultToolkit()
	...
	public BufferedImage getGrey() {
		ColorConvertOp ccp = new ColorConvertOp(ColorSpace.getInstance(1003),
				null);
	...
	public BufferedImage getBlur() {
		float[] data = { 0.1111F, 0.1111F, 0.1111F, 0.1111F, 0.1111F, 0.1111F,
	...
	public BufferedImage getRotate() {
		AffineTransformOp atop = new AffineTransformOp(
				AffineTransform.getRotateInstance(3.141592653589793D,
	...
	public static void main(String[] args) throws IOException {
		FileInputStream fin = new FileInputStream("E:\\imgcode22.jpg");
		...
		File file = new File("E:\\imgcode-22.jpg");
